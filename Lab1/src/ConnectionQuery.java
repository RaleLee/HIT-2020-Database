...
public class ConnectionQuery extends GUI{
    private static List<String> query = new ArrayList<String>();

    public static void main(String[] args){
        Connect conn = new Connect();
        ConnectionQuery gui = new ConnectionQuery("ConnectionQuery", conn);
        return;
    }

    public ConnectionQuery(String title, Connect connect) {
        // build sql sentence
        buildSQL();
        // 创建 JFrame 实例
...
//        });
    ...
    private void buildSQL(){
        // 查询有3个以上护士参与的手术的手术信息
        String sql1 = "Select surgery.pID, surgery.type, surgery.date, Count(nurse.nID)" +
                "\nFrom surgery, nurse, help " +
                "\nWhere surgery.sID=help.sID and nurse.nID=help.nID " +
                "\nGroup by help.sID Having Count(*) > 3;";
        // 查询每个护士负责的患者信息
        String sql2 = "Select nurse.name, patient.name, patient.age, patient.sex " +
                "\nFrom nurse, patient, care " +
                "\nWhere care.nID=nurse.nID and care.bID=patient.bID;";
        // 查询每个医生所在的科室名及医生信息
        String sql3 = "Select dname, doctor.name, doctor.sex, doctor.age, doctor.title, doctor.position " +
                "\nFrom department, doctor " +
                "\nWhere department.deID=doctor.deID;";
        query.add(sql1);
        query.add(sql2);
        query.add(sql3);
    }
